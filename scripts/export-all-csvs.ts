...
async function exportAllData() {  if (!fs.existsSync(OUTPUT_DIR)) {
  ...
  if (owners) {
    const uniqueOwners = Array.from(new Map(
      owners.map(o => [(o as any).profiles.email, o])
    ).values());
    ...
    const ownersCSV = arrayToCSV(
      ['email', 'nombre_completo', 'telefono', 'user_id'],
      ownerRows
    );
  ...
  const { data: ownerList } = await supabase
    .from('businesses')
    .select('owner_id');
  ...
  if (appointments) {
    ...
    const appointmentsCSV = arrayToCSV(
      ['inicio', 'fin', 'estado', 'precio', 'moneda', 'pago', 'negocio', 'servicio', 'cliente', 'email_cliente', 'empleado'],
      appointmentRows
    );
