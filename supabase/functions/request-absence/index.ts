...
serve(async (req) => {
  ...
  try {
    ...
    const { data: business, error: businessError } = await supabaseClient
      .from('businesses')
      .select('vacation_days_per_year, allow_same_day_absence, require_absence_approval, max_advance_vacation_request_days, owner_id')
      .eq('id', businessId)
      .single();
    ...
    if (!business.allow_same_day_absence && start.getTime() === today.getTime() && absenceType !== 'emergency') {
    ...
    if (absenceType === 'vacation') {
      ...
      if (daysAdvance > business.max_advance_vacation_request_days) {
        throw new Error(`Vacaciones solo pueden solicitarse con máximo ${business.max_advance_vacation_request_days} días de anticipación`);
      }
      if (start < today) {
    ...
    if (absenceType === 'vacation') {
      ...
      const yearsWorked = Math.floor((today.getTime() - hireDate.getTime()) / (1000 * 60 * 60 * 24 * 365));
      ...
      if (daysRequested > daysRemaining) {
        throw new Error(`Solo tiene ${daysRemaining} días de vacaciones disponibles. Solicitó ${daysRequested} días.`);
      }
    }
