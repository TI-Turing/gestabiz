-- Migration: Fix missing image_url and duration_minutes in get_client_dashboard_data RPC
-- Date: 2026-03-12
-- Description: The migration 20251201120001_fix_category_rpc_groupby.sql accidentally
--   removed 'image_url' and 'duration_minutes' from the service object returned by the RPC.
--   This caused appointment cards in the client dashboard to not show service background images.
--   Also restores 'google_maps_url' in the location object.
...
-- Add comment
COMMENT ON FUNCTION get_client_dashboard_data IS 'Consolidated RPC for client dashboard data including appointments with full service data (image_url, duration_minutes), location data (google_maps_url), business categories and stats.';
