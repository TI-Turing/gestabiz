...
-- Eliminar función existente con la firma correcta (3 parámetros)
DROP FUNCTION IF EXISTS get_client_dashboard_data(uuid, text, text) CASCADE;
-- Recrear función con logo_url y banner_url (3 parámetros)
...
-- Comment
COMMENT ON FUNCTION get_client_dashboard_data(uuid, text, text) IS 'Returns dashboard data for client including logo_url and banner_url for favorites and frequent businesses';
