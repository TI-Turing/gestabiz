-- =============================================================================
-- Migration: Add region/department filtering to get_client_dashboard_data RPC
-- =============================================================================
-- Description: Adds p_preferred_region_name parameter to filter businesses by
--              department/state when no specific city is selected
-- Date: 2025-11-27
-- =============================================================================

-- 1. Drop existing function
DROP FUNCTION IF EXISTS public.get_client_dashboard_data(UUID, TEXT) CASCADE;
-- 2. Recreate with region filtering support
...
BEGIN
  ...
  WITH location_stats AS (
    SELECT
      b.id AS business_id,
      (SELECT city FROM locations WHERE business_id = b.id LIMIT 1) AS location_city,
  ...
  INTO v_suggestions
  FROM businesses b
  INNER JOIN location_stats loc ON b.id = loc.business_id
...
'Consolidated client dashboard payload with favorites, city AND region/department filtering, and sorted suggestions. 
Parameters:
- p_client_id: UUID of the client
- p_preferred_city_name: City name for filtering (optional, filters locations.city)
