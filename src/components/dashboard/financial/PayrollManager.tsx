...
export function PayrollManager({ businessId }: Readonly<PayrollManagerProps>) {
  ...
  const [paymentForm, setPaymentForm] = useState<Partial<PayrollPayment>>({
    payment_period_start: new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0],
  ...
  const calculateCommissions = async (employeeId: string, startDate: string, endDate: string): Promise<number> => {
    ...
    try {
      ...
      if (config.commission_base === 'transactions' || config.commission_base === 'both') {
        ...
        totalCommissionable += (transactions || []).reduce((sum, tx) => sum + (tx.total_amount || 0), 0);
      }
  ...
  const calculateDeductions = (salaryBase: number, otherDeductions: Array<{ name: string; amount: number }> = []) => {
  ...
  const handleOpenPayment = async (employee: Employee) => {
    ...
    const startDate = paymentForm.payment_period_start || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
    ...
    // Calculate totals
    const total_earnings = employee.salary_base + commissions + prestaciones.cesantias + 
                          prestaciones.prima + prestaciones.vacaciones + 
                          prestaciones.intereses_cesantias + (paymentForm.other_earnings || 0);

    const total_deductions = deductions.health_deduction + deductions.pension_deduction + 
  ...
  const handleGeneratePDF = async (payment: PayrollPayment) => {
    ...
    try {
      ...
      await exportPayrollToPDF(pdfData, `Comprobante_Nomina_${payment.employee_name}_${payment.payment_period_end}.pdf`);
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="employees" className="space-y-4">
              {loading && <div className="text-center py-8 text-muted-foreground">Cargando empleados...</div>}
                            ...
                            <div className="flex-1">
                              ...
                              <div className="mt-2 flex gap-4 text-sm">
                                <div>
                                  ...
                                  <span className="font-medium">{formatCOP(employee.salary_base)}</span>
                                </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h4 className="font-medium text-foreground">{payment.employee_name}</h4>
                              <span className={`text-xs px-2 py-0.5 rounded-full ${getPaymentStatusClass(payment.status)}`}>
                                {getPaymentStatusLabel(payment.status)}
                              </span>
                            </div>
                            <p className="text-sm text-muted-foreground mt-1">
                              Período: {new Date(payment.payment_period_start).toLocaleDateString('es-CO')} - {new Date(payment.payment_period_end).toLocaleDateString('es-CO')}
                            </p>
                            <div className="mt-2 grid grid-cols-3 gap-4 text-sm">
                              <div>
                                <span className="text-muted-foreground">Devengado: </span>
                                <span className="font-medium">{formatCOP(payment.total_earnings)}</span>
                              </div>
                              <div>
                                <span className="text-muted-foreground">Deducciones: </span>
                                <span className="font-medium text-red-600">-{formatCOP(payment.total_deductions)}</span>
                              </div>
                              <div>
                                <span className="text-muted-foreground">Neto: </span>
                                <span className="font-bold text-foreground">{formatCOP(payment.net_payment)}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex gap-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setConfigForm({...configForm, commission_rate: parseFloat(e.target.value) || 0})}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setConfigForm({...configForm, commission_base: value as PayrollConfig['commission_base']})}
                  >
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => setConfigForm({...configForm, calculate_prestaciones: checked})}
                  />
                ...
                <div className="space-y-3 pl-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Ahorro para terminación de contrato</p>
                    </div>
                    <Switch
                      checked={configForm.cesantias_enabled}
                      onCheckedChange={(checked) => setConfigForm({...configForm, cesantias_enabled: checked})}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setConfigForm({...configForm, prima_enabled: checked})}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Provisión para vacaciones anuales</p>
                    </div>
                    <Switch
                      checked={configForm.vacaciones_enabled}
                      onCheckedChange={(checked) => setConfigForm({...configForm, vacaciones_enabled: checked})}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setConfigForm({...configForm, intereses_cesantias_enabled: checked})}
                    />
                  </div>
                </div>
              )}
      ...
      <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedEmployee?.full_name} - Período de pago
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="bg-muted p-4 rounded-lg space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatCOP(paymentForm.intereses_cesantias || 0)}</span>
                </div>
                
                <div className="border-t pt-2 flex justify-between font-medium">
                  <span>TOTAL DEVENGADO:</span>
                  <span className="text-green-600">{formatCOP(paymentForm.total_earnings || 0)}</span>
                </div>
                ...
                <div className="border-t pt-2 flex justify-between font-medium">
                  ...
                  <span className="text-red-600">-{formatCOP(paymentForm.total_deductions || 0)}</span>
                </div>
