      ...
      <Card className="border-destructive/50">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mb-4">{t('settings.dangerZone.deactivate.subtitle')}</p>

            <div className="bg-muted/30 rounded-lg p-4 mb-4">
              <p className="text-sm font-medium mb-2">{t('settings.dangerZone.deactivate.whatHappens')}</p>
              <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
                {['markedInactive', 'sessionClosed', 'futureAppointments', 'noLogin', 'dataPreserved'].map(key => (
                  <li key={key}>{t(`settings.dangerZone.deactivate.consequences.${key}`)}</li>
                ))}
              </ul>
              <div className="mt-3 pt-3 border-t border-border">
                ...
                <p className="text-xs text-muted-foreground">{t('settings.dangerZone.deactivate.contactSupport')}</p>
              </div>
            </div>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={handleCloseDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteStep === 1 ? t('settings.dangerZone.delete.step1Description') : t('settings.dangerZone.delete.step2Description')}
            </DialogDescription>
          </DialogHeader>
          ...
          {deleteStep === 1 && (
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="confirm-email" type="email" placeholder={t('settings.dangerZone.delete.emailPlaceholder')} value={confirmEmail} onChange={(e) => setConfirmEmail(e.target.value)} className="w-full" />
              </div>
              <div className="flex items-start space-x-2">
                <Checkbox id="understand" checked={understandConsequences} onCheckedChange={(checked) => setUnderstandConsequences(checked as boolean)} />
          ...
          {deleteStep === 2 && (
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="confirm-text" type="text" placeholder={t('settings.dangerZone.delete.confirmPlaceholder')} value={confirmText} onChange={(e) => setConfirmText(e.target.value)} className="w-full font-mono" />
              </div>
              <div className="bg-muted/50 rounded-lg p-3">
                <p className="text-xs text-muted-foreground mb-2">{t('settings.dangerZone.delete.confirmDetails')}:</p>
                <ul className="text-xs text-muted-foreground space-y-1">
                  {[
                    `${t('settings.dangerZone.delete.accountLabel')}: ${user.email}`,
                    `${t('settings.dangerZone.delete.profileLabel')}: ${user.name}`,
                    `${t('settings.dangerZone.delete.rolesLabel')}: ${user.roles?.length || 0} ${t('settings.dangerZone.delete.activeLabel')}`,
                    `${t('settings.dangerZone.delete.appointmentsLabel')}: ${t('settings.dangerZone.delete.cancelledAuto')}`,
                  ].map((text, i) => (
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={handleCloseDialog} disabled={isDeleting} className="w-full sm:w-auto">
              {t('common.actions.cancel')}
            </Button>
            {deleteStep === 1 ? (
              <Button variant="destructive" onClick={handleProceedToStep2} disabled={confirmEmail.toLowerCase() !== user.email.toLowerCase() || !understandConsequences} className="w-full sm:w-auto">
                {t('settings.dangerZone.delete.continue')}
              </Button>
            ) : (
              <Button variant="destructive" onClick={handleDeleteAccount} disabled={confirmText !== 'DESACTIVAR CUENTA' || isDeleting} className="w-full sm:w-auto">
                {isDeleting ? (
                  <><Loader2 className="h-4 w-4 mr-2 animate-spin" />{t('settings.dangerZone.delete.deactivating')}...</>
                ) : (
                  <><AlertCircle className="h-4 w-4 mr-2" />{t('settings.dangerZone.delete.deactivateNow')}</>
                )}
              </Button>
            )}
          </DialogFooter>
