...
describe('FileUpload Component', () => {
  ...
  it('should reject invalid file types', async () => {
    ...
    await waitFor(() => {
      expect(mockOnUploadError).toHaveBeenCalledWith(
        expect.stringContaining('tipo no permitido')
      );
    });
  });
  ...
  it('should reject when exceeding max file count', async () => {
    ...
    await waitFor(() => {
      expect(mockOnUploadError).toHaveBeenCalledWith(
        expect.stringContaining('máximo 2 archivos')
      );
    });
  });
  ...
  it('should handle upload errors gracefully', async () => {
    // Mock upload failure
    vi.mocked(require('@/lib/supabase').supabase.storage.from).mockReturnValueOnce({
      upload: vi.fn().mockResolvedValue({
        data: null,
