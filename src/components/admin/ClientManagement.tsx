...
export default function ClientManagement(props: Readonly<ClientManagementProps>) {
  ...
  const getStatusBadge = (status: Client['status']) => {
    switch (status) {
      case 'active': return <Badge variant="default" className="bg-green-500">{t('admin.clientManagement.status.active')}</Badge>
      case 'inactive': return <Badge variant="secondary">{t('admin.clientManagement.status.inactive')}</Badge>
      case 'blocked': return <Badge variant="destructive">{t('admin.clientManagement.status.blocked')}</Badge>
      default: return <Badge variant="outline">{t('admin.clientManagement.status.unknown')}</Badge>
    }
  }

  const handleSendFollowUp = (client: Client) => {
    if (!client.whatsapp) { toast.error(t('admin.clientManagement.whatsapp.missing')); return }
  ...
  return (
    ...
    <Input placeholder={t('admin.clientManagement.search_placeholder')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
        </div>
  <Select value={statusFilter} onValueChange={onStatusChange}>
    <SelectTrigger className="w-full sm:w-[200px]"><SelectValue placeholder={t('admin.clientManagement.filter_by_status')} /></SelectTrigger>
          <SelectContent>
  ...
  <Tabs value={activeTab} onValueChange={onTabChange} className="space-y-4">
    ...
    <TabsTrigger value="all">{t('admin.clientManagement.tabs.all')} ({filteredClients.length})</TabsTrigger>
    <TabsTrigger value="recurring">{t('admin.clientManagement.tabs.recurring')} ({stats.recurring})</TabsTrigger>
    <TabsTrigger value="at_risk">{t('admin.clientManagement.tabs.at_risk')} ({stats.atRisk})</TabsTrigger>
    <TabsTrigger value="lost">{t('admin.clientManagement.tabs.lost')} ({stats.lost})</TabsTrigger>
        </TabsList>
        <TabsContent value={activeTab} className="space-y-4">
          {filteredClients.length === 0 ? (
      <Card><CardContent className="py-16 text-center"><Users className="h-12 w-12 mx-auto mb-4 text-muted-foreground opacity-50" /><h3 className="text-lg font-medium mb-2">{t('admin.clientManagement.empty.title')}</h3><p className="text-muted-foreground">{searchTerm ? t('admin.clientManagement.empty.description_search') : t('admin.clientManagement.empty.description_category')}</p></CardContent></Card>
          ) : (
