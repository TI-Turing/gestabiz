...
import { LayoutDashboard, MapPin, Briefcase, Users, FileText, Shield, CreditCard, BriefcaseBusiness, ShoppingCart, Calendar, CalendarOff, Box, Wallet, UserCheck, BarChart3, Megaphone } from 'lucide-react'
...
const MarketingManager = lazy(() => import('./marketing/MarketingManager').then(m => ({ default: m.MarketingManager })))
...
export function AdminDashboard({ 
  ...
  const sidebarItems = useMemo(() => {
    const items = [
      ...
      planItem('marketing',   'Marketing',                               <Megaphone className="h-5 w-5" />),
  ...
  const renderContent = () => {
    ...
    switch (activePage) {
      ...
      case 'marketing':
        return planWrap('marketing', <MarketingManager businessId={business.id} />)
      case 'permissions':
