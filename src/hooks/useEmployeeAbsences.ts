...
export function useEmployeeAbsences(businessId: string) {
  ...
  const { data: absences = [], isLoading: loading, error: absencesError, refetch: refetchAbsences } = useQuery({
  ...
  const validateWorkDays = useCallback(
    async (startDate: string, endDate: string): Promise<{ isValid: boolean; invalidDays: string[] }> => {
  ...
  return {
    absences,
    vacationBalance,
    loading: loading || loadingBalance || requestAbsenceMutation.isPending || cancelAbsenceMutation.isPending,
