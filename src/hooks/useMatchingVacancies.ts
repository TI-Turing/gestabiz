...
export function useMatchingVacancies() {
  ...
  const fetchMatchingVacancies = useCallback(async (
    userId?: string,
    filters?: VacancyFilters
  ): Promise<MatchingVacancy[]> => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (filters?.position_type) {
        filtered = filtered.filter((v: MatchingVacancy) => v.position_type === filters.position_type);
      }

      if (filters?.experience_level) {
        filtered = filtered.filter((v: MatchingVacancy) => v.experience_required === filters.experience_level);
      }

      if (filters?.min_salary !== undefined) {
        filtered = filtered.filter((v: MatchingVacancy) => 
          v.salary_min !== null && v.salary_min !== undefined && v.salary_min >= (filters.min_salary || 0)
        );
      }

      if (filters?.max_salary !== undefined) {
        filtered = filtered.filter((v: MatchingVacancy) => 
          v.salary_max !== null && v.salary_max !== undefined && v.salary_max <= (filters.max_salary || Infinity)
        );
      }
  ...
  const filterByScore = useCallback((minScore: number) => {
