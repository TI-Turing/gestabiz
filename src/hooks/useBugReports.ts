...
interface UseBugReportsReturn {
  ...
  uploadEvidence: (bugReportId: string, file: File, description?: string) => Promise<BugReportEvidence | null>
...
export function useBugReports(): UseBugReportsReturn {
  ...
  const getBugReportById = useCallback(async (id: string): Promise<BugReport | null> => {
    if (!user) return null
  ...
  const getEvidences = useCallback(async (bugReportId: string): Promise<BugReportEvidence[]> => {
    if (!user) return []
  ...
  // Eliminar reporte
  const deleteBugReport = useCallback(async (id: string): Promise<boolean> => {
  ...
  // Eliminar evidencia
  const deleteEvidence = useCallback(async (id: string, filePath: string): Promise<boolean> => {
    if (!user) return false
