...
export function filterAppointments(appointments: Appointment[], filters: AppointmentFilter): Appointment[] {
...
export function sortAppointments(appointments: Appointment[], sortBy: 'date' | 'client' | 'status' | 'priority', order: 'asc' | 'desc' = 'asc'): Appointment[] {
...
export function getUpcomingAppointments(appointments: Appointment[], limit: number = 5): Appointment[] {
  const now = new Date()
  
  return appointments
    .filter(apt => 
      apt.status === 'scheduled' && 
  new Date(apt.start_time || (apt.date && apt.startTime ? `${apt.date}T${apt.startTime}` : '')) > now
    )
    .sort((a, b) => {
  const dateA = new Date(a.start_time || (a.date && a.startTime ? `${a.date}T${a.startTime}` : ''))
  const dateB = new Date(b.start_time || (b.date && b.startTime ? `${b.date}T${b.startTime}` : ''))
      return dateA.getTime() - dateB.getTime()
    })
    .slice(0, limit)
}

export function getAppointmentsByDateRange(appointments: Appointment[], start: Date, end: Date): Appointment[] {
  return appointments.filter(apt => {
  const src = apt.start_time || (apt.date && apt.startTime ? `${apt.date}T${apt.startTime}` : undefined)
  if (!src) return false
  const appointmentDate = new Date(src)
...
export function groupAppointmentsByDate(appointments: Appointment[]): { [date: string]: Appointment[] } {
  return appointments.reduce((groups, appointment) => {
    const date = appointment.date || (appointment.start_time ? appointment.start_time.split('T')[0] : undefined)
...
export function getAvailableTimeSlots(appointments: Appointment[], date: string, workingHours: { start: string, end: string }, slotDuration: number = 60): string[] {
