...
export function hasPermission(
  ...
  return userPermissions.some(
    (p) => p.permission === requiredPermission && p.is_active &&
...
export function hasBusinessRole(
  ...
  return businessRoles.some(
    (br) => br.business_id === businessId && br.role === role && br.is_active
  )
}
...
export function getUserBusinessRole(
  ...
  const role = businessRoles.find(
    (br) => br.business_id === businessId && br.is_active
  )
 ...
 */
export function canProvideServices(
  businessRoles: BusinessRole[],
  businessId: string
): boolean {
