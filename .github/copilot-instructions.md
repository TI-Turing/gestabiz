...
14. **Validar Edge Functions antes de alterar columnas**: Al agregar/renombrar/eliminar columnas, buscar en `supabase/functions/` las funciones que consultan esa tabla y actualizar en el mismo commit ⭐ NUEVO (Abr 2026)
...
11. **Edge Functions y columnas inexistentes**: PostgREST falla silenciosamente con HTTP 500 si un `select()` pide una columna que no existe en la tabla — NUNCA asumir que una columna existe; siempre verificar con `information_schema.columns` antes de escribir un query en una Edge Function o al crear migraciones que eliminen columnas. Caso real: `profiles!client_id(id, full_name, email, phone, whatsapp)` → `whatsapp` no existe → 500 en cada llamada durante 3 sesiones.
